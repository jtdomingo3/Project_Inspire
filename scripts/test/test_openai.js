...
async function testAssistant() {
  try {
    ...
    const convRes = await fetch(`${API_URL}/assistant/conversations`, {
      method: 'POST',
      headers,
      body: JSON.stringify({ title: 'Test Run - OpenAI Debug', model: 'gpt-4o-mini' })
    });
    ...
    console.log('3. Sending query to OpenAI (this may take a few seconds)...');
    const queryRes = await fetch(`${API_URL}/assistant/conversations/${convId}/query`, {
      method: 'POST',
      headers,
      body: JSON.stringify({ question: 'Hi what can you do', model: 'gpt-4o-mini' })
    });
