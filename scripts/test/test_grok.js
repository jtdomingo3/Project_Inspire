...
async function testGrok() {
  try {
    ...
    const convRes = await fetch(`${API_URL}/assistant/conversations`, {
      method: 'POST',
      headers,
      body: JSON.stringify({ title: 'Test Run - Grok Debug', model: 'grok-2-mini' })
    });
    ...
    console.log('3. Sending query to Grok (this may take a few seconds)...');
    const queryRes = await fetch(`${API_URL}/assistant/conversations/${convId}/query`, {
      method: 'POST',
      headers,
      body: JSON.stringify({ question: 'Hi Grok, what can you do?', model: 'grok-2-mini' })
    });
